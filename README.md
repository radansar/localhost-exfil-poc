...
| Channel | How | Works on |
|---|---|---|
| `<img>` HTTP | Identifier in URL query string; Safari loads HTTP images from localhost even from an HTTPS page | Safari |
| TLS SNI | Identifier hex-encoded in the SNI hostname; extracted from the raw TLS ClientHello before any cert check | Safari + Chrome |
